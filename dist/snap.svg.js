...
/**
 * Created by throrin19 on 26/03/14.
 */
Snap.plugin(function (Snap, Element, Paper, glob) {
    ...
    var Hammer = (function(window, undefined) {
        'use strict';
...
//        }
    ...
    elproto.drag = function drag(onmove, onstart, onend, move_scope, start_scope, end_scope) {
        if (!arguments.length) {
            var origTransform;
            return this.drag(function (dx, dy) {
                this.attr({
            ...
            var dragstart = function(evt) {
                ...
                dragmove = function(evt) {
                    evt.gesture.preventDefault();
                    onmove(evt.gesture.deltaX, evt.gesture.deltaY, evt.gesture.center.pageX, evt.gesture.center.pageY, evt);
            ...
            if(onstart){
                Hammer(this.node).on('dragstart', dragstart);
            }
            if(onmove){
                Hammer(this.node).on('drag', dragmove);
            }
            if(onend){
                Hammer(this.node).on('dragend', dragend);
            }
